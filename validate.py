...
if not d["is_mat_comp"] and not d["is_cg"]:
    print(f'\n  ERROR: Benchmark must be run with exactly one of --mat_comp and --cg')
    valid = False
...
if d["is_mat_comp"]:
    if d['re'] is None:
        print("Error: could not find relaive norm of error in {:}".format(
            consolefile ) )
    ...
    if valid and d["ndof"] == 10000 and d["p"] == 3:
        ...
        if abs(d["y_norm"] - 1.141577508) > 1e-09 or abs(d["z_norm"] - 1.141577508) > 1e-09:
            print(f'  ERROR: INCORRECT VALUES REPORTED                  ')
            valid = False
        print(f'  ===========================================')

    if valid and abs(d["re"]) > 1e-14:
        print(f'   Relative error norm too large')
        valid = False
